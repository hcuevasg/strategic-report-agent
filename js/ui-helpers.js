...
function startProgress() {
  ...
  const st = document.getElementById('progressStep');
  ...
  progressTimer = setInterval(() => {
    if (i < STEPS.length) {
      f.style.width = STEPS[i].pct + '%';
      st.textContent = STEPS[i].msg;
      p.textContent = STEPS[i].pct + '%';
      i++;
    }
...
function stopProgress(ok) {
  ...
  if (ok) {
    f.style.width = '100%';
    st.textContent = t('uiCompleted');
    p.textContent = '100%';
    setTimeout(() => document.getElementById('progressSection').classList.add('hidden'), 1500);
  } else {
...
function setLoading(on) {
  ...
  if (on) {
    ...
    b.innerHTML =
      ...
      '<span class="font-[Manrope] text-xs uppercase tracking-[0.2em]">Procesando...</span>';
  } else {
    ...
    b.innerHTML =
      ...
      '<span class="font-[Manrope] text-xs uppercase tracking-[0.2em]">Generar Informe Ejecutivo</span>';
  }
}
...
function hidePreview() {
  ...
  document.getElementById('methodCards').classList.remove('hidden');
}
...
function setDot(s) {
  ...
  const l = document.getElementById('statusDotLabel');
  d.classList.remove('connected');
  if (s === 'ok') {
    d.style.background = '#10b981';
    d.style.boxShadow = '0 0 8px rgba(16,185,129,0.6)';
    d.classList.add('connected');
    l.textContent = t('uiConnected');
  } else if (s === 'no') {
    d.style.background = '#E74243';
    d.style.boxShadow = '0 0 8px rgba(187,0,20,0.6)';
    l.textContent = t('uiError');
  } else {
    d.style.background = '#64748b';
    d.style.boxShadow = 'none';
    l.textContent = t('uiOffline');
  }
}
...
function flash(m) {
  showStatus(m);
  setTimeout(() => {
    if (result) showStatus(t('uiReportReadyEdit'));
  }, 2500);
}

function loadSample() {
  document.getElementById('inputText').value =
    'Tenemos un problema con las tiendas de México. Los eventos de pérdida han bajado un 15% en Q1 2026 comparado con Q4 2025, pero no sabemos si es porque mejoró la prevención o porque dejaron de reportar.\n\n' +
