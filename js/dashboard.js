...
async function openDashboard() {
  ...
  let token = getStatsToken();
  if (!token) {
    token = prompt(
      'Ingresa tu Stats Token (primeros 20 caracteres de tu ANTHROPIC_API_KEY o tu STATS_TOKEN):'
    );
    if (!token) return;
    setStatsToken(token);
  }
  ...
  overlay.style.cssText =
    'position:fixed;inset:0;z-index:9999;background:rgba(4,22,39,0.7);display:flex;align-items:center;justify-content:center;animation:fadeIn .3s ease;';
  overlay.innerHTML = `
...
function renderDashboard(data) {
  ...
  const totalAbuse = abuse.reduce((sum, d) => {
    return (
      sum +
      Object.entries(d)
        .filter(([k]) => !k.endsWith('_details') && k !== 'date')
  ...
  if (totalAbuse > 0) {
    ...
    Object.entries(abuseTypes)
      .sort((a, b) => b[1] - a[1])
      .forEach(([k, v]) => {
        const labels = {
          no_token: 'No auth token',
          invalid_token: 'Invalid token',
          origin_rejected: 'Origin blocked',
          rate_limited: 'Rate limited',
          report_brute_force: 'Report brute force',
        };
        h += `<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
  ...
  h += `<div style="display:flex;justify-content:space-between;align-items:center;padding-top:12px;border-top:1px solid #E0E3E5;">
    ...
    <button onclick="clearStatsToken();closeDashboard();" style="font-family:Inter,sans-serif;font-size:9px;color:${A.SGRAY};background:none;border:none;cursor:pointer;text-decoration:underline;">Reset token</button>
  </div>`;
