...
function tp(k) {
  return (
    (pptxI18n[typeof currentLang !== 'undefined' ? currentLang : 'es'] || pptxI18n.es)[k] ||
    pptxI18n.es[k] ||
    k
  );
}
...
- Mínimo 10 slides, máximo 16 (sin contar cover y closing).`,
      ...
      (fullText, chunk) => {
        ...
        if (count > slidesDetected) {
          ...
          const lastTitle = titleMatch
            ? titleMatch[titleMatch.length - 1]
                .replace(/"action_title"\s*:\s*"/, '')
                .replace(/"$/, '')
            : '';
    ...
    let slideData;
    try {
      slideData = parseModelJSON('slides', txt);
    } catch (e) {
      throw new Error('IA no devolvió JSON de slides válido: ' + e.message);
    }
    ...
    function addLogo(sl) {
      if (logoBase64)
        sl.addImage({ data: 'image/png;base64,' + logoBase64, x: 12.1, y: 0.1, w: 1.0, h: 0.39 });
    }
    ...
    function addNote(sl, text) {
      if (text) sl.addNotes(text);
    }

    function soWhatBox(sl, text, y) {
      /* no-op */
    }
    function srcNote(sl, t) {
      if (t)
        sl.addText(tp('source') + ': ' + t, {
          x: M,
          y: 6.9,
          w: 8,
          h: 0.2,
    ...
    slides.forEach((s, sIdx) => {
      ...
      try {
        ...
        if (s.type === 'cover') {
          ...
          sl.addText(s.action_title || result.title, {
            x: 0.55,
            y: 1.45,
            w: leftW - 0.8,
            h: 2.8,
          ...
          sl.addText(new Date().toLocaleDateString('es-CL', { year: 'numeric', month: 'long' }), {
            x: 3.28,
            y: 5.75,
            w: 3,
            h: 0.3,
          ...
          sl.addText('ALTO · Strategic Insights', {
            x: rX + 0.25,
            y: 6.2,
            w: rW - 0.4,
            h: 0.25,
        ...
        if (s.type === 'closing') {
          ...
          sl.addText(s.action_title || 'Gracias', {
            x: 0.55,
            y: 1.6,
            w: cRX - 0.8,
            h: 2.5,
          ...
          if (logoWhiteBase64)
            sl.addImage({
              data: 'image/png;base64,' + logoWhiteBase64,
              x: cRX + 0.5,
              y: 2.8,
              w: 2.8,
              h: 1.1,
        ...
        if (s.type === 'toc') {
          ...
          items.forEach((item, i) => {
            ...
            sl.addShape('roundRect', {
              x: M,
              y: iy + (rowH - circleSize) / 2,
              w: circleSize,
              h: circleSize,
              fill: { color: i === 0 ? A.RED : A.TBLUE },
              rectRadius: 0.12,
            });
        ...
        if (s.type === 'divider') {
          ...
          if (s.subheading)
            sl.addText(s.subheading.toUpperCase(), {
              x: 0.55,
              y: 2.15,
              w: dRX - 0.8,
              h: 0.3,
          ...
          if (s.items && s.items.length) {
            ...
            s.items.slice(0, maxItems).forEach((item, i) => {
              sl.addText(i + 1 + '. ' + (typeof item === 'string' ? item : item.title || ''), {
                x: 0.72,
                y: miniY + i * itemH,
                w: dRX - 1.2,
                h: itemH,
        ...
        const accentColor =
          s.type === 'risks' ? A.RED : s.type === 'opportunities' ? A.TBLUE : A.RED;
        ...
        if (s.section_label) {
          sl.addText(s.section_label.toUpperCase(), {
            x: M,
            y: cy,
            w: eLW,
            h: 0.2,
        ...
        if (vis === 'stat_callouts' && s.data_points && s.data_points.length) {
          ...
          dp.forEach((d, i) => {
            ...
            const numFontSize =
              val.length <= 3 ? 52 : val.length <= 5 ? 40 : val.length <= 8 ? 32 : 24;
            sl.addShape('rect', {
              x: cx + 0.08,
              y: cy,
              w: colW - 0.16,
          ...
          if (normBar.length && cats.length && cats.length > 0) {
            ...
            sl.addChart(pptx.charts.BAR, normBar, {
              ...
              barDir: 'col',
              barGapWidthPct: 60,
              catAxisLabelColor: A.BODY,
              valAxisLabelColor: A.BODY,
              ...
              dataLabelColor: A.NAVY,
              showDataLabel: true,
              dataLabelPosition: 'outEnd',
              catAxisTruncate: false,
              catGridLine: { style: 'none' },
              valGridLine: { color: A.MGRAY, style: 'dash', size: 0.5 },
              valAxisHidden: false,
              valAxisMajorUnit: null,
              plotAreaBorderColor: A.WHITE,
              chartColors: singleSeries
                ? [A.NAVY, A.RED, A.NAVY, A.RED, A.NAVY, A.RED, A.NAVY, A.RED]
                : [A.NAVY, A.RED, A.TBLUE],
              varyColors: singleSeries,
              showLegend: !singleSeries,
              legendPos: 'b',
              legendFontSize: 8,
          ...
          const chartH = Math.min(5.4, 7.5 - cy - 0.8);
          const chartW = eLW * 0.85;
          ...
          const donutW = eLW * 0.55;
          const donutH = 3.8;
          const donutX = M + (eLW - donutW) / 2;
          ...
          const steps = s.columns.slice(0, 5);
          const n = steps.length;
          const arrowW = 0.3;
          const boxW = (eLW - arrowW * (n - 1)) / n;
          const boxH = Math.min(3.8, 7.5 - cy - 1.2);
          const boxY = cy + 0.5;
          ...
          steps.forEach((step, i) => {
            const bx = M + i * (boxW + arrowW);
            const sc = stepColors[i % 3];
          ...
          cols.forEach((col, i) => {
            ...
            items.forEach((item, j) => {
              ...
              itemTextParts.push({
                text: '\u25B8 ',
                options: { fontSize: 10, color: colColors[i % 3], bold: true },
              });
          ...
          phases.forEach((ph, i) => {
            ...
            items.forEach((item, j) => {
              ...
              itemTextParts.push({
                text: '\u25B8 ',
                options: { fontSize: 10, color: i === 0 ? A.RED : A.NAVY, bold: true },
              });
              itemTextParts.push({ text: item, options: { fontSize: 10, color: A.BODY } });
            });
            if (itemTextParts.length)
          ...
          pils.forEach((p, i) => {
            ...
            items.forEach((item, j) => {
              ...
              itemTextParts.push({
                text: '\u25B8 ',
                options: { fontSize: 10, color: pilColors[i % 4], bold: true },
              });
              itemTextParts.push({ text: item, options: { fontSize: 10, color: A.BODY } });
            });
            if (itemTextParts.length)
          ...
          const matH = 7.5 - cy - 0.55;
          const matW = eLW;
          const halfW = matW / 2 - 0.06;
          const halfH = matH / 2 - 0.06;
          const qColors = [A.TBLUE, A.RED, A.NAVY, A.NAVY];
          const qFills = ['EBF2FA', 'FFF0F0', A.LGRAY, A.LGRAY];
          // Axis center lines
          const midX = M + matW / 2;
          const midY = cy + matH / 2;
          sl.addShape('rect', {
            x: M,
            y: midY - 0.015,
            w: matW,
          ...
          q.forEach((qd, i) => {
            ...
            qItems.forEach((item, j) => {
              ...
              qItemParts.push({
                text: '▸ ',
                options: { fontSize: 9, color: qColors[i], bold: true },
              });
              qItemParts.push({ text: item, options: { fontSize: 9, color: A.BODY } });
            });
            if (qItemParts.length)
              sl.addText(qItemParts, {
                x: qx + 0.15,
                y: qy + 0.55,
                w: halfW - 0.3,
                h: halfH - 0.7,
          ...
          pts.forEach(pt => {
            ...
            if (pt.type === 'total') {
              ...
              bots.push(Math.min(running, running + v));
              tops.push(Math.max(running, running + v));
              if (pt.type !== 'total') running += v;
            }
          });
          ...
          pts.forEach((pt, i) => {
            ...
            const by =
              chartBottom - (barTop - dataMin) * scale - (barH > rawH ? (barH - rawH) / 2 : 0);
          ...
          const chartH = Math.min(5.0, 7.5 - cy - 0.9);
          const chartW = eLW * 0.95;
          ...
          const gridW = eLW;
          const gridH = Math.min(7.5 - cy - 0.5, rows * 1.8);
          ...
          items.forEach((item, i) => {
            ...
            sl.addShape('roundRect', {
              ...
              h: cellH - cellPad * 2,
              fill: { color: A.LGRAY },
              line: { color: 'DDDFE2', width: 0.5 },
              rectRadius: 0.08,
            });
            // Numbered circle
            const circR = 0.26;
            const circX = cx + cellPad + 0.18;
            const circY = cy2 + cellPad + 0.18;
            sl.addShape('ellipse', {
              x: circX,
              y: circY,
              w: circR * 2,
              h: circR * 2,
              fill: { color: circleColors[i % 3] },
            });
            sl.addText(String(i + 1), {
              x: circX,
              y: circY,
            ...
            sl.addText(item.title || '', {
              x: titleX,
              y: cy2 + cellPad + 0.15,
              w: cellW - cellPad * 2 - circR * 2 - 0.34,
              h: 0.42,
              fontSize: 10,
              fontFace: 'Calibri',
              color: A.NAVY,
            ...
            sl.addText(item.description || '', {
              x: cx + cellPad + 0.14,
              y: cy2 + cellPad + 0.72,
              w: cellW - cellPad * 2 - 0.2,
              h: cellH - cellPad * 2 - 0.82,
              fontSize: 9,
              fontFace: 'Calibri',
              color: A.BODY,
              lineSpacingMultiple: 1.25,
          ...
          pts.forEach((pt, i) => {
            ...
            sl.addShape('rect', {
              ...
              w: barW,
              h: barH,
              fill: { color: levelColors[i % 7] },
            });
            // Value label on bar (left side, white text)
            sl.addText(String(pt.value || ''), {
              x: bx + 0.12,
              y: by,
              w: Math.min(1.2, barW * 0.3),
              h: barH,
              fontSize: 11,
              fontFace: 'Calibri',
              color: A.WHITE,
          ...
          if (s.bullets && s.bullets.length) {
            ...
            s.bullets.forEach((b, j) => {
              ...
              leftParts.push({
                text: '\u25B8 ',
                options: { fontSize: 11, color: accentColor, bold: true },
              });
              leftParts.push({ text: b, options: { fontSize: 11, color: A.BODY } });
            });
          }
          if (leftParts.length)
            sl.addText(leftParts, {
              x: M,
              y: contentTop,
              w: leftColW,
              h: contentH,
          ...
          if (s.type === 'executive_summary') {
            ...
            s.bullets.forEach((b, j) => {
              ...
              parts.push({
                text: '\u25B8 ',
                options: { fontSize: 12, color: accentColor, bold: true },
              });
              parts.push({ text: b, options: { fontSize: 11, color: A.BODY } });
            });
            ...
            const hasMoreContent =
              !!(s.data_points && s.data_points.length) || (s.bullets && s.bullets.length);
            if (!hasMoreContent) {
              ...
              const availH =
                ...
                cy -
                0.6 -
                (s.data_points && s.data_points.length ? 1.7 : 0) -
                (s.bullets && s.bullets.length ? s.bullets.length * 0.5 + 0.2 : 0);
              sl.addText(s.body_text, {
                x: M,
                y: cy,
                w: eLW,
                h: Math.min(availH, 2.5),
                fontSize: 13,
                fontFace: 'Calibri',
                color: A.BODY,
                lineSpacingMultiple: 1.6,
          ...
          if (s.data_points && s.data_points.length) {
            ...
            dp.forEach((d, i) => {
              ...
              sl.addShape('rect', {
                ...
                w: dpW - 0.1,
                h: 0.05,
                fill: { color: i % 2 === 0 ? A.NAVY : A.RED },
              });
              sl.addText(String(d.value), {
                x: dx + 0.05,
                y: cy + 0.15,
                w: dpW - 0.1,
                h: 0.6,
        ...
        slideErrors.push({
          idx: sIdx + 1,
          title: s.action_title || '(sin título)',
          err: slideErr.message,
        });
      }
    });
    if (slideErrors.length) {
      const msg = slideErrors
        .map(e => '· Slide ' + e.idx + ' "' + e.title + '": ' + e.err)
        .join('\n');
      showError(
        'Advertencia — ' +
          slideErrors.length +
          ' slide(s) con error (el resto se generó correctamente):\n' +
          msg
      );
    }
    ...
    if (result.information_gaps && result.information_gaps.length) {
      ...
      sl.addShape('rect', { x: 0, y: 0, w: 0.08, h: 7.5, fill: { color: A.NAVY } });
      // Header
      sl.addText('Anexo: Áreas Pendientes de Validación', {
        x: M,
        y: 0.25,
        w: 8,
        h: 0.6,
      ...
      sl.addShape('rect', { x: M, y: 0.88, w: 1.0, h: 0.04, fill: { color: A.RED } });
      sl.addText(
        'Los siguientes puntos requieren información adicional para completar el análisis.',
        {
          x: M,
          y: 1.05,
          w: CW * 0.7,
      ...
      sl.addText('BACKUP · PARA DISCUSIÓN', {
        x: M,
        y: 6.8,
        w: 4,
        h: 0.25,
    ...
    await pptx.writeFile({
      fileName: 'Informe_ALTO_' + new Date().toISOString().slice(0, 10) + '.pptx',
    });
